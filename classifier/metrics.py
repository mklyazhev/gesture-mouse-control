...
def get_metrics(
    ...
    scores = {
        "accuracy": accuracy(predicts_labels, targets, task=task, average=average, num_classes=num_classes),
        "f1_score": f1_score(predicts_labels, targets, task=task, average=average, num_classes=num_classes),
        "precision": precision(predicts_labels, targets, task=task, average=average, num_classes=num_classes),
        "recall": recall(predicts_labels, targets, task=task, average=average, num_classes=num_classes),
    }

    if mode == "test":
        scores["roc_auc"] = auroc(predicts, targets, task=task, average=average, num_classes=num_classes)
    ...
    if mode == "valid" or mode == "test":
        if target == "leading_hand":
            ...
            class_names = conf.dataset.gestures

        cm = confusion_matrix(predicts, targets, task=task, num_classes=num_classes)
