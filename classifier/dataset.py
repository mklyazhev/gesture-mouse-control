...
from classifier.constants import IMAGES
...
class GestureDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, is_train: bool, conf: DictConfig, transform: Compose = None, is_test: bool = False) -> None:
        ...
        self.labels = {
            label: num for (label, num) in zip(self.conf.dataset.gestures, range(len(self.conf.dataset.gestures)))
        }
    ...
    def __read_annotations(self, subset: int = None) -> pd.DataFrame:
        ...
        path_to_json = os.path.expanduser(self.conf.dataset.annotations) # extra?
        for target in self.conf.dataset.gestures:
            # TODO: if target == "no_gesture": continue -
            #  если это ничего не поломает в обучении
            target_tsv = os.path.join(path_to_json, f"{target}.json")
            if os.path.exists(target_tsv):
                json_annotation = json.load(open(target_tsv))
